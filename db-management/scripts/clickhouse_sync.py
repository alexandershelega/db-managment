...
def run(config_dir: str, dry_run: bool):
    ...
    templates = read_template_databases(str(Path(config_dir) / "clickhouse_databases.txt"))
    cfg = load_connections(config_dir, "clickhouse")
    handler = ClickHouseHandler(cfg, dry_run)
